...
WMFConverter.prototype.toCanvas = function(inputWmfFile, outputWmfFile) {
    ...
    parseWMF(new DataView(inputWmfFile, 0), canvas);
    
    const buffer = canvas.toBuffer('image/svg+xml')
    console.log("image", buffer.toString())
    fs.writeFileSync(outputWmfFile, buffer)
	...
	function parseWMF(dv, canvas) {
		...
		console.log("parameters", mtVersion, mtSize, mtNoObjects, mtMaxRecord, mtMaxRecord, mtNoParameters)
		// if (mtType != 1 || mtHeaderSize != 9) {
        //     console.log("error", mtType, mtHeaderSize)
		// 	throw new UserException("Invalid file format.");
        // }
        console.log("dataview_", dv.buffer, dv.byteLength);
		console.log("here3")
        ...
        let counter = 1;
		while (true) {
            console.log("counter", counter, offset)
			...
			case RECORD_SET_WINDOW_EXT_EX: {
                ...
                let inMemCanvas = createCanvas(1200, 600)
